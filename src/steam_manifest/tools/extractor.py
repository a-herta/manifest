...
class RepositoryExtractor:
    ...
    def process_branch(self, branch_name: str) -> None:
        ...
        if is_chinese(name):
            ...
            final_name = combine_multilingual_names(english, chinese_name, japanese_name)
        else:
            english_name = english or name
            chinese_name = simplified_chinese or traditional_chinese
            final_name = combine_multilingual_names(english_name, chinese_name, japanese)

        # Get update time
        update_timestamp = depots.get('branches', {}).get('public', {}).get('timeupdated', '')
        if update_timestamp:
        ...
        achievement_files = [f for f in os.listdir(self.repo_path) if f.endswith('.bin')]
