    ...
    for remote_ref in origin.refs:
        ...
        if remote_branch_name in repo.heads:
            ...
            if tracking_branch != remote_ref:
                ...
                print(f"Updated local branch {remote_branch_name} to track {remote_ref}")
            continue
        ...
        try:
            ...
            local_branch.set_tracking_branch(remote_ref)
            print(f"Created local branch {remote_branch_name} tracking {remote_name}/{remote_branch_name}")
        except GitCommandError as e:
