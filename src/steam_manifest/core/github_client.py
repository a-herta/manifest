...
class GitHubClient:
    ...
    def api_request(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            with httpx.Client(
                ...
                if response.status_code == 429:  # Rate limit
                    ...
                    if reset_time:
                        ...
                        self.logger.warning(f"⚠️ Rate limit reached, retrying in {wait_time:.0f} seconds")
                ...
                with self.lock:
                    self.logger.debug(f"📥 Received response: {len(str(json_data))} bytes")
            ...
            self.logger.error(f"🔧 HTTP request failed: {e.response.status_code} - {url}")
            return None
    ...
    def get_file_content(self, repo: str, branch: str, path: str) -> Optional[Dict[str, Any]]:
