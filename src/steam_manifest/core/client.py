...
class SteamManifestClient:
    ...
    def verify_prerequisites(self) -> Tuple[Optional[Path], Optional[str], Optional[str]]:
        ...
        for repo in repos:
            try:
                ...
                if not branch_info or "commit" not in branch_info:
                    self.logger.debug(f"📝 No matching branch found in repository: {repo}")
                    continue
    ...
    def process_repository(self, repo: str, branch: str, steam_path: Path, is_dlc: bool = False):
        ...
        try:
            ...
            if all(task.successful() for task in tasks) and not is_dlc:
                ...
                self.logger.info(f"📅 Successfully processed: {self.app_info}, last updated: {commit_date}")
                
        except Exception as e:
            self.logger.error(f"❌ Error processing repository {repo} branch {branch}: {str(e)}")
            raise
    
    def _process_manifest_file(self, repo: str, branch: str, path: str, steam_path: Path):
            ...
            if self.fixed_mode:
                manifest_list = sorted(
                    [
                        ...
                        for parts in (manifest.split("_") for manifest in self.manifests)
                    ],
                    key=lambda x: x[0],
                )
